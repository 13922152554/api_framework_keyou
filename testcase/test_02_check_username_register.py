...
# 获取判断用户名是否注册接口测试数据
check_username_register_data = member_data['check_username_register']


class TestCheckUsernameRegister(MemberApi):
    """ 判断用户名是否注册接口 测试案例层 """

    @allure.story('判断用户名是否已注册')
    @allure.title('{test_data[title]}')
    @pytest.mark.parametrize("test_data", check_username_register_data)
    def test_check_username_register(self, test_data, conn_db):
        """
        判断用户名是否注册接口测试案例
        ...
        # 发送判断用户名是否注册请求
        result = self.check_username_register_api(**test_data['data'])
        ...
        if test_data['sql']:
            # sql语句数据替换
            test_data['sql'] = test_data['sql'].replace('#username#', test_data['data']['username'])
